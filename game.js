...
let racing = false;
let stopStop = true; // to get rid of the default velocity = 0
let startGame = false;
let slowDownActivate = false;
let keyIsAvailable = false;
let maxCollection = false;
let pushPunch = true;
// let Fall = false;
let lose = false;
let healthBarEmpty = false;
let instruction = false;
let aboutMe = false;
let inspiration = false;
let reference = false;
let anim = false;
let bossForwards = false;
let finishBoss = false;
let loading = true;
...
let intro;
let punchSound;
let punchFinishSound;
let energyPickedSound;
let heartPickedSound;
let punchPickedSound;
let startCarSound;
let raceStartSound;
let carCrashSound;
// menu images
...
function setup() {
    ...
    gp = createGraphics(window.innerWidth, 600); // to use tint()
    gp.background(200, 200, 100, 0); // make gp bg transparent

    gp2 = createGraphics(window.innerWidth, 600); // to use tint()
    gp2.background(200, 200, 100, 0); // make gp bg transparent

    gp3 = createGraphics(window.innerWidth, 600); // to use tint()
    gp3.background(200, 200, 100, 0); // make gp bg transparent

    gp4 = createGraphics(window.innerWidth, 600); // to use tint()
    ...
    energyPickedSound.setVolume(0.3);
    carCrashSound.setVolume(0.4);
    carCrashSound.setVolume(0.6);
...
function raceCount() {
    ...
    if (counter > 0) {
        ...
        raceStart = false;
        startGame = true; // start drawing all the game elements when spacebar is hit once
    }
}
...
function draw() {
    ...
    // currentFrame = (currentFrame + 1) % numFrames; // speed up all the turtles
    background(51);
    if (loading) {
...
function keyPressed() {
    if (!touchScreen || !mobileResize) {
        ...
        if (punch.length >= 5) {
            ...
            console.log("Can't use puch skill");
        }
    }
}
